...
<div class="middle-container">
    ...
    'Send SMS',
     array('controller' => 'users', 'action' => 'sendsms', $thisuserdetail['User']['id']), array('class' => 'sms'));?>
           <?php  echo $this->Html->link(
    'Send E-mail'.$this->Html->tag('span', null, array('class' => 'glyphicon glyphicon-chevron-right')),
     array('controller' => 'users', 'action' => 'sendmail', $thisuserdetail['User']['id']), array('class' => 'mail active','escape' => false));?>
          </article>
        ...
        <div class="row">         
          <section class="col-md-10 col-md-offset-1 col-sm-12 col-sx-12">
            <div class="send-sms-sectin send-message-section">
			 ...
			 ?>
             <h3>Sending Invite Message to <?php echo $hisher;?> Email Address: <strong><?php  echo $this->Html->link(
    $thisuserdetail['Matrimonial']['first_name'].' '.$thisuserdetail['Matrimonial']['last_name'],
     array('controller' => 'users', 'action' => 'profiledetail', $thisuserdetail['User']['id']), array('class' => 'mail'));?></strong></h3>
			 ...
			 if(isset($msg)){
				 ...
				 ?>
             <div class="form">
              <?php echo $this->Form->create('User', array('action' => 'sendmail'.'/'.$thisuserdetail['Matrimonial']['user_id'],'id'=>'UserSendmailForm')); ?>
              <div class="col-md-12 col-sm-6 input-row">
              <?php echo $this->Form->input('sendmail.subject', array('label' => 'Subject:','div' => false,'placeholder' => 'Your Subject','default' => '') ); ?>
              <?php echo $this->Form->input('sendmail.userid', array('type' => 'hidden','value' => $thisuserdetail['Matrimonial']['user_id']) ); ?>
              </div>
              <div class="col-md-12 col-sm-6 input-row">
              <?php echo $this->Form->input(
              'sendmail.message', array('label' => 'Message:','type' => 'textarea','default'=>'','placeholder'=>'Type Your Message............','rows'=>4));?>
              </div><br/><br/>
               <div class="col-md-12 col-sm-6 input-row" style="margin-top:10px;">
              <?php echo $this->Form->button('Send Message', array('type' => 'submit' ,'class'=>'btn'));
              echo $this->Form->end();
		      ?>
              </div>
              <?php 
...
jQuery(document).ready(function(){
	jQuery('#UserSendmailForm').validate({
	    rules: {
	      "data[sendmail][subject]": {
	      	required: true
	      },
	     "data[sendmail][message]": {
	        required: true,
			maxlength: 200,
			maxlength: 40
	      }
	   ...
	   messages: {
		    "data[sendmail][subject]": {
				required: "Mail Subject is not empty.",
			},
			"data[sendmail][message]": {
				required: "Mail Message is not empty",
			}
		},
